...
//  act={"start":1528285827585,"timeout":60,"prev_balance":0,"current_balance":0,"total_amount":74.86000318514343,"type":"Buy","my_orders":{},"amount_changed":0,"trades":[],"triggerRunning":true,"done":false}
    ...
    calc_new_order: function (mname, act, market, remaining_amount) {

        if (mname == 'USDT_BTC' ) {
            vol_frac = Math.floor (100000000 * rlexec_config.vol_rez * remaining_amount /* * u.midprice (market) */ / rlexec_config.full_volume)
            l.d('mname = ' + mname + ' market = ' + market + ' vol_frac = int (100000000 * ' + rlexec_config.vol_rez + ' * ' + remaining_amount  + /*' * ' + u.midprice (market)  +*/ ' / ' + rlexec_config.full_volume + ') = ' + vol_frac + "(" + (100000000 * rlexec_config.vol_rez * remaining_amount  /** u.midprice (market)*/ / rlexec_config.full_volume) + ")");
        } else {
            vol_frac = Math.floor (100000000 * rlexec_config.vol_rez * remaining_amount  * u.midprice (market) / rlexec_config.full_volume)
            l.d('mname = ' + mname + ' market = ' + market + ' vol_frac = int (100000000 * ' + rlexec_config.vol_rez + ' * ' + remaining_amount  + ' * ' + u.midprice (market)  + ' / ' + rlexec_config.full_volume + ') = ' + vol_frac + "(" + (100000000 * rlexec_config.vol_rez * remaining_amount  * u.midprice (market) / rlexec_config.full_volume) + ")");
        }
...
//        time_frac = Math.floor (rlexec_config.time_rez * (1. - (Date.now() - act['start']) / act['timeout']))
        ...
        l.d('time_frac = int (' + rlexec_config.time_rez + ' * (' + Date.now() + ' - ' + act['start'] + ') / ' + act['timeout'] * 1000. + ') = ' + time_frac);
        if (time_frac > rlexec_config.time_rez - 1) {
            ...
            if (action < 0 || action > rlexec_config['noof_actions']) {
                l.v('Crashing - action = ' + action + ' at ' + new Error().stack);
                process.exit(1);
                throw new RangeError('Crashing - action = ' + action);
            }
        }

        l.i('RLExecConfab - v:' + vol_frac + ', t:' + time_frac + ' => a:' + action);
