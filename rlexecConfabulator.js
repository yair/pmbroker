...
//    policy_dir: './rlexec_output/1544853989/',
    policy_dir: './rlexec_output/1550472569/',
    full_volume: 10000000, // 100mB
    full_period: 60000,  // 60s         *** TESTING *** !!! UNUSED !!!
//    full_period: 180000,  // 180s
//    full_period: 360000,  // 360s
//    volumes_file: 'volumes.json',
    volumes_file: 'volumes.poloniex.360s.1550389548000.json',
...
function init_volumes (rlc) {
	fs.readFile (rlc['policy_dir'] + rlc['volumes_file'], 'utf8', function (err, data) {
        if (err) {
            u.crash(`Error reading volumes file ${rlc['volumes_file']}: ${err}`);
/*            l.e(`Error reading volumes file ${rlc['volumes_file']}: ${err}`);
            process.exit(1);*/
        }
//        l.i(rlc['volumes_file'] + " dump: " + data);
        ...
        l.i('rleConfab: read ' + Object.keys(json).length + ' volumes.');
    });
}
...
function init_policies (rlc) {
    ...
    fs.readdir (rlc['policy_dir'], function (err, items) {
        if (err) {
            u.crash('Error reading policy dir ' + rlc['policy_dir'] + ": " + err);
/*            l.e('Error reading policy dir ' + rlc['policy_dir'] + ": " + err);
            process.exit(1);*/
            return;
        }
...
//                console.log("4 = market = " + market + " policies=" + policies);
                if (err) {
                    l.e('Error reading policy file ' + fn + ': ' + err);
                    return;
                }
//                console.log(fn + " contents: " + data + ' (market=' + market + ', item=' + items[market] + ')'); 
        ...
        l.i('rleConfab: read ' + items.length + ' policies.');
    });
    return policies;
}
...
//    return .5 * (getSortedOB(market['ob_bids']).reverse()[0][0] + getSortedOB(market['ob_asks'])[0][0]);
        ...
        if (act['type'] != 'Sell') {
            u.crash("Crashing - Unknown act type " + act['type']);
/*            l.e("Crashing - Unknown act type " + act['type'] + ' at ' + new Error().stack);
            process.exit(1);*/
...
//        console.log("no_deeper_than = (" + volumes[act['mname']] + " / " + rlexec_config['volume_divizor'] + ") * " + 2. ** (-action) + " = " + no_deeper_than);
        l.d("no_deeper_than = " + volumes[act['mname']] + " * " + 2. ** (-action) + " = " + no_deeper_than + 'bsat');
        no_deeper_than /= 100000000 * 0.5 * (parseFloat(our0) + parseFloat(their0));
        l.d('no_deeper_than /= 100000000 * ' + (0.5 * (parseFloat(our0) + parseFloat(their0))) + ' = ' + no_deeper_than + 'alt');
...
*/
        ...
        if (act['type'] == 'Buy') {
            if (our0 + PRICE_RESOLUTION >= their0) {
                l.v('ALU_NEAR Buy: our0 = ' + our0);
                return our0;
            } else {
                l.v('ALU_NEAR Buy: price = ' + our0 + ' + ' + PRICE_RESOLUTION);
                return our0 + PRICE_RESOLUTION;
            }
        } else {
            if (our0 - PRICE_RESOLUTION <= their0) {
                l.v('ALU_NEAR Sell: our0 = ' + our0);
                return our0;
            } else {
                l.v('ALU_NEAR Sell: price = ' + our0 + ' - ' + PRICE_RESOLUTION);
                return our0 - PRICE_RESOLUTION;
            }
        }
        ...
        if (act['type'] == 'Buy') {
            if (Math.floor((our0 + their0) / 2) >= their0) {
                l.v('ALU_MID Buy: price = their0 - rez = ' + their0 - PRICE_RESOLUTION);
                return their0 - PRICE_RESOLUTION;
            } else {
                l.v('ALU_MID Buy: price = (our0 + their0) / 2 = ' + ((our0 + their0) / 2));
                return ((our0 + their0) / 2);
            }
        } else {
            if (Math.floor((our0 + their0) / 2) <= their0) {
                l.v('ALU_MID Sell: price = their0 + rez = ' + (their0 + PRICE_RESOLUTION));
                return their0 + PRICE_RESOLUTION;
            } else {
                l.v('ALU_MID Sell: price = (our0 + their0) / 2 = ' + ((our0 + their0) / 2));
                return ((our0 + their0) / 2);
            }
        }
    } else if (action == rlexec_config['action_lu']['ALU_FAR']) {
        if (act['type'] == 'Buy') {
            l.v('ALU_FAR Buy: price = their0 - rez = ' + their0 - PRICE_RESOLUTION);
            return their0 - PRICE_RESOLUTION;
        } else {
            l.v('ALU_FAR Sell: price = their0 + rez = ' + their0 + PRICE_RESOLUTION);
            return their0 + PRICE_RESOLUTION;
        }
        ...
        u.crash("Crashing - invalid action " + JSON.stringify(action));
/*        l.e("Crashing - invalid action " + JSON.stringify(action) + ' at ' + new Error().stack);
        process.exit(1);*/
...
//    return .5 * (getSortedOB(market['ob_bids']).reverse()[0][0] + getSortedOB(market['ob_asks'])[0][0]);
//        our_obkv = u.getSortedOB(u.remove_self_from_ob(market['ob_bids'], act)).reverse();
        our_obkv = u.getSortedOB(u.remove_self_from_ob(act['ob_bids'], act)).reverse();
//        their_obkv = u.getSortedOB(market['ob_asks']);
        their_obkv = u.getSortedOB(act['ob_asks']);
    } else {
        if (act['type'] != 'Sell') {
            u.crash("Crashing - Unknown act type " + act['type']);
/*            l.e("Crashing - Unknown act type " + act['type'] + ' at ' + new Error().stack);
...
//        our_obkv = u.getSortedOB(u.remove_self_from_ob(market['ob_asks'], act));
        our_obkv = u.getSortedOB(u.remove_self_from_ob(act['ob_asks'], act));
//        their_obkv = u.getSortedOB(market['ob_bids']).reverse();
        their_obkv = u.getSortedOB(act['ob_bids']).reverse();
    }
    our0 = parseFloat(our_obkv[0][0]);
    their0 = parseFloat(their_obkv[0][0]);
    if (action <= rlexec_config['action_lu']['ALU_00150625AV']) {
        no_deeper_than = volumes[act['mname']] * 2. ** (-action);
        l.v("no_deeper_than = " + volumes[act['mname']] + " * " + 2. ** (-action) + " = " + no_deeper_than + 'bsat');
        no_deeper_than /= 100000000 * 0.5 * (parseFloat(our0) + parseFloat(their0));
        l.v('no_deeper_than /= 100000000 * ' + (0.5 * (parseFloat(our0) + parseFloat(their0))) + ' = ' + no_deeper_than + 'alt');
        price = parseFloat(u.get_depth_price (no_deeper_than, our_obkv));
        if (act['type'] == 'Buy') {
            if (price + PRICE_RESOLUTION >= their0) { // Don't overstep into their ob too soon
                return price;
...
/*       if action >= ALU_MID_NEAR and action <= ALU_MID_FAR:
		...
		if (action == rlexec_config['action_lu']['ALU_MID_NEAR']) {
			wanted_price = (3 * our0 + their0) / 4;
			l.v('wanted_price = (3 * our0 + their0) / 4 = ' + wanted_price);
		} else if (action == rlexec_config['action_lu']['ALU_MID']) {
			wanted_price = (our0 + their0) / 2;
			l.v('wanted_price = (our0 + their0) / 2 = ' + wanted_price);
		} else if (action == rlexec_config['action_lu']['ALU_MID_FAR']) {
			wanted_price = (our0 + 3 * their0) / 4;
			l.v('wanted_price = (our0 + 3 * their0) / 4 = ' + wanted_price);
		}
        if (act['type'] == 'Buy') {
            if (wanted_price >= their0) {
                l.v('ALU_MID Buy: price = their0 - rez = ' + their0 - PRICE_RESOLUTION);
                ...
                l.v('ALU_MID Buy: price = wanted_price = ' + wanted_price);
                return wanted_price;
            }
        } else {
            if (wanted_price <= their0) {
                ...
                l.v('ALU_MID Sell: price = wanted_price = ' + wanted_price);
                return wanted_price;
            }
        }
    } else if (action == rlexec_config['action_lu']['ALU_FAR']) {
		...
		if (action == rlexec_config['action_lu']['ALU_BR_0125']) {
			...
			l.v('ALU_BR_0125: no_deeper_than = vol / 8 = ' + no_deeper_than);
		} else {
			no_deeper_than = remaining_amount * (action - rlexec_config['action_lu']['ALU_BR_0125']) / 4;
			l.v('ALU_BR: no_deeper_than = vol * ' + (action - rlexec_config['action_lu']['ALU_BR_0125']) + ' / 4 = ' + no_deeper_than);
		}
        no_deeper_than /= 100000000 * 0.5 * (parseFloat(our0) + parseFloat(their0));
        l.v('no_deeper_than /= 100000000 * ' + (0.5 * (parseFloat(our0) + parseFloat(their0))) + ' = ' + no_deeper_than + 'alt');
        price = parseFloat(u.get_depth_price (no_deeper_than, their_obkv));
		l.v('raw price = ' + price + ' (their0 = ' + their0 + ')');
        if (act['type'] == 'Buy') {
			l.v('price = ' + price + ' - ' + PRICE_RESOLUTION + ' = ' + (price - PRICE_RESOLUTION));
			return price - PRICE_RESOLUTION;
		} else {
			l.v('price = ' + price + ' + ' + PRICE_RESOLUTION + ' = ' + (price + PRICE_RESOLUTION));
			return price + PRICE_RESOLUTION;
...
module.exports = {
    ...
    init: function (config, cb) { //_polo) {
//        polo = _polo; // unused
        c = config;
        init_volumes (rlexec_config);
        policies = init_policies (rlexec_config);
        l.d('RLExecConfabulator initialized and ready.');
        cb();
    },

    get_next_price: function (act) {

        return action_price (act.mname, act, act.action, act.remaining_amount);
    },
...
//  act={"start":1528285827585,"timeout":60,"prev_balance":0,"current_balance":0,"total_amount":74.86000318514343,"type":"Buy","my_orders":{},"amount_changed":0,"trades":[],"triggerRunning":true,"done":false}
    ...
    calc_new_order: function (mname, act, market, remaining_amount) {   // <--- s'what we use

        if (mname == 'USDT_BTC' ) {
            vol_frac = Math.floor (100000000 * rlexec_config.vol_rez * remaining_amount /* * u.midprice (market) */ / rlexec_config.full_volume)
            l.v('mname = ' + mname + ' market = ' + JSON.stringify(market));
            l.d('vol_frac = int (100000000 * ' + rlexec_config.vol_rez + ' * ' + remaining_amount  + /*' * ' + u.midprice (market)  +*/ ' / ' + rlexec_config.full_volume + ') = ' + vol_frac + "(" + (100000000 * rlexec_config.vol_rez * remaining_amount  /** u.midprice (market)*/ / rlexec_config.full_volume) + ")");
        } else {
            vol_frac = Math.floor (100000000 * rlexec_config.vol_rez * remaining_amount  * u.midprice (market) / rlexec_config.full_volume)
            l.v('mname = ' + mname + ' market = ' + JSON.stringify(market));
            l.d('vol_frac = int (100000000 * ' + rlexec_config.vol_rez + ' * ' + remaining_amount  + ' * ' + u.midprice (market)  + ' / ' + rlexec_config.full_volume + ') = ' + vol_frac + "(" + (100000000 * rlexec_config.vol_rez * remaining_amount  * u.midprice (market) / rlexec_config.full_volume) + ")");
        ...
        if (vol_frac < 0 || vol_frac > rlexec_config.vol_rez - 1) {
            u.crash('Crashing - vol_rez = ' + rlexec_config.vol_rez);
/*            l.e('Crashing - vol_rez = ' + rlexec_config.vol_rez + ' at ' + new Error().stack);
            process.exit(1);*/
...
//        time_frac = Math.floor (rlexec_config.time_rez * (1. - (Date.now() - act['start']) / act['timeout']))
        ...
        l.d('time_frac = int (' + rlexec_config.time_rez + ' * (' + Date.now() + ' - ' + act['start'] + ' = ' + (Date.now()-act['start']) + ') / ' + act['timeout'] * 1000. + ') = ' + time_frac);
        if (time_frac > rlexec_config.time_rez - 1) {
            l.d('action = ALU_MARKET');
            action = rlexec_config['action_lu']['ALU_MARKET'];
            if (time_frac > rlexec_config.time_rez + 3) {
                u.crash('Crashing - unable to execute market order');
/*                l.e('Crashing - unable to execute market order at ' + new Error().stack);
                process.exit(1);*/
            }
        } else if (time_frac < 0) {
            u.crash('Crashing - time_rez = ' + rlexec_config.time_rez);
/*            l.e('Crashing - time_rez = ' + rlexec_config.time_rez + ' at ' + new Error().stack);
            process.exit(1);*/
            ...
            if (action < 0 || action > rlexec_config['noof_actions']) {
                u.crash('Crashing - action = ' + action);
/*                l.e('Crashing - action = ' + action + ' at ' + new Error().stack);
                process.exit(1);*/
                throw new RangeError('Crashing - action = ' + action);
            }
        }

        l.i('RLExecConfab (' + act['type'] + ' ' + mname + ') - v:' + vol_frac + ', t:' + time_frac + ' => a:' + action);
...
//        ret = {};
        ret        = {      'mname':    mname,
                            ...
                            'market_order': action == rlexec_config['action_lu']['ALU_MARKET'],
                            'rlexec_state': { 'v': vol_frac, 't': time_frac, 'a': action } };
        l.v("Confabulator returning " + JSON.stringify (ret));
        return ret;
    }
